...
The BebeLM bridge is typed as an embedding provider. The vectors below come from
the local GGUF model and are cached as RDS because they are deterministic for the
...
assignments against direct graph edges. The embeddings are cached because this
is a real BebeLM pass over the ontology labels.
...
ontology_label_embeddings <- ducksemantics_cache_rds(
  file.path(cache_dir, "readme-bebel-ontology-label-embeddings.rds"),
  function() ducksemantics_embed(embedding_provider, ontology_label_terms$label)
)
...
rfmalloc_runtime <- Rfmalloc::open_fmalloc(
  file.path(cache_dir, "readme-ontology-clusters.fmalloc"),
  mode = "scratch",
  size_gb = 2
)
